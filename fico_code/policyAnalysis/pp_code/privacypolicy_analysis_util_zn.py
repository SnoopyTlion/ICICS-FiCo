...
from Constants_zn import WWWWWH, NOT_DATA, SHARE_ACTION, COLLECT_ACTION, TPL_LIST
...
def preprocess_sen(sen):
    ...
    # 将一个句子分割成单词列表，将其中的乱码字符去除，因为要提取五元组，所以长度小于5的直接去除掉
    # {数据实体、动作、{数据类型}、{数据接收者}、{neg}}
    pattern = re.compile(r'[\uf001-\uf0ff]')
...
def token_list_to_dep_info(tokenResult, depResult):
    ...
    # depInfoWords返回5元组，【依存关系，依存的单词位置，依存的具体单词，单词本身位置，单词本身】
    return depInfoWords
...
# 传入分词结果，成分分析，完整的hanlp的结果,合并名词短语返回
...
def check_data(data):
    for tmp in data:
        if '服务' in tmp or tmp in ['你', '你的', '您的', '您']:
            return False
        for d in NOT_DATA:
...
def extract_use_pattern_en(sentence, wordDepInfo, token):
    data = []
    pattern1 = r'(.*)你(.*)[提供|存储|保存|给](.*)信息(.*)'

    if '信息如下：' in sentence or '包括' in sentence:
        ...
        for i in ['提供', '存储', '保存', '给']:
            if i in sentence:
                ...
                if re_tmp is not None:
                    ...
                    if list(set(data)) == ["信息"]:
                        return []
                    tmp = [[["用户"], [i], list(set(data)), [], []]]
                    return tmp
    return []


def judge_in_verb(verb):
    for t in SHARE_ACTION:
        if t in verb:
            return 1
    for t in COLLECT_ACTION:
        if t in verb:
            return 2
    return 0
...
def analyze_host_app_share(hanlp_results):
    ...
    for i in range(len(hanlp_token)):
        ...
        if hanlp_pos[i] == 'VV' or hanlp_pos[i] == 'VERB':
            ...
            # 有2个词语list，两类词汇
            if (judge_in_verb(verb) == 1 or judge_in_verb(verb) == 2) and verb_flag[i] != 1:
                actor, action, data, entity, neg = [], [verb], [], [], []
                if judge_in_verb(verb) == 1:
                    action = ['分享']
                elif judge_in_verb(verb) == 2:
                    action = ['收集']

                # 根部动词下标
                root_verb_idx = i
                verb_flag[i] = 1
                if any(v in verb for v in ['提供', '给出', '输入', '给与']) and token[i - 1] in ['你', '用户', '用户们','您']:
                    actor.append('用户')
                ...
                for s_idx in range(beg_flag, len(wordDepInfo)):
                    ...
                    if sactor[1] - 1 == root_verb_idx:
                        ...
                        elif cur_tok in ['我们', '你们', '你','您'] and '提供' in verb:
                            entity.append(cur_tok)
                        elif cur_tok in ['我们', '你们', '你','您']:
                            actor.append(cur_tok)
                        elif sactor[0] == 'accmod' or sactor[0] == 'mmod' or sactor[0] == 'advmod':
                            if cur_tok in ['不', '从不', '从未', '不应']:
                                neg.append('negative')
                            if cur_tok == 'if':
                                actor, action, data, entity, neg = [], [], [], [], []
                                beg_flag = s_idx + 1
                        ...
                        if sactor[0] == 'pcomp' or sactor[0] == 'conj' or sactor[0] == 'ccomp':
                            ...
                            if hanlp_pos[sactor[1] - 1] == 'VV' or hanlp_pos[sactor[1] - 1] == 'VERB':
                                ...
                                actor.extend(tmp_actor)
                        elif sactor[0] == 'dep' and sactor[2] in ['不']:
                            neg.append('negative')
                            tar_idx = sactor[1] - 1
                            ...
                            for ti in range(s_idx, -1, -1):
                                ...
                                if wordDepInfo[ti][1] - 1 == tar_idx and wordDepInfo[ti][0] == 'dobj' and (
                                        wordDepInfo[ti][-1]).lower() in ['你', '您', '用户']:
                                    actor.append(wordDepInfo[ti][-1])
                                    break
                    # add ner info here
...
def analyze_host_app_pp_zn(file_name, save_name, data_name, hanlp_mtl, print_flag=True):
    data_set = set()
    ...
    ded_list = []
    fs = open(save_name, 'w', encoding='utf-8')
    for sen in sentence:
        # preprocess sentence
    ...
    with open(data_name, 'w', encoding='utf-8') as f:
        ...
        #         if len(DED[i][2]) != 0:
        #             print(DED)
        #             result = Process_DED(DED[i][2])
        #             for r in result:
        #                 if r in ded_list or len(r) == 0:
        #                     continue
        #                 else:
        #                     ded_list.append(r)
        #                     fs.write(str(r) + '\n')
